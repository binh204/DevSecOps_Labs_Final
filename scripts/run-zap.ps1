...
# 2. Khởi chạy ZAP Daemon Container
...
SPIDER_ID=$(curl -s "http://localhost:8080/JSON/spider/action/scan/?url=$TARGET_URL/&recurse=true" | grep -o '"scan":"[0-9]*"' | cut -d'"' -f4)
if [ -z "$SPIDER_ID" ]; then
    SPIDER_ID="0"
fi
...
while true; do
    PROGRESS=$(curl -s "http://localhost:8080/JSON/spider/view/status/?scanId=$SPIDER_ID" | grep -o '"status":"[0-9]*"' | cut -d'"' -f4)
    if [ -z "$PROGRESS" ]; then
        PROGRESS="100"
    fi
...
curl -s "http://localhost:8080/OTHER/core/other/xmlreport/" --output ./reports/zap/report.xml 2>/dev/null || true
curl -s "http://localhost:8080/OTHER/core/other/htmlreport/" --output ./reports/zap/report.html 2>/dev/null || true
curl -s "http://localhost:8080/OTHER/core/other/jsonreport/" --output ./reports/zap/report.json 2>/dev/null || true
