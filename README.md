...
- **Attack Payload (via Web Browser):**
...
- **Detection & Response Flow (Multi-layered Protection):**
  ...
  3. **Wazuh Manager Correlation & Action:** Wazuh Manager parses and correlates the incoming logs, matching them against **Rule ID `100002`** (Suricata Alert) and **Rule ID `31105`** (Nginx XSS Alert). Since both alerts match critical security thresholds, Wazuh Manager triggers the **Active Response** mechanism to instruct VM1 to drop the attacker's IP via `iptables` for 10 minutes.

- **Wazuh Custom Rule Configuration (`/var/ossec/etc/rules/local_rules.xml`):**
  ```xml
  <group name="ids,suricata,">
    <rule id="100002" level="10">
      <if_sid>86601</if_sid>
      <field name="event_type">^alert$</field>
      <field name="alert.signature">^CUSTOM XSS Attempt Detected$</field>
      <description>Wazuh Alert: Suricata Custom XSS Attack Blocked by Active Response</description>
    </rule>
  </group>
  ```
...
- **Detection & Response Flow (Multi-layered Protection):**
  ...
  3. **Wazuh Manager Correlation:** Wazuh Manager correlates the logs, matching **Rule ID `100003`** (Suricata custom alert) and **Rule ID `31106`** (Nginx SQLi alert), raising the severity levels to `10`, logging them to the SIEM dashboard, and triggering Active Response to blacklist the source IP.
...
- **Wazuh Custom Rule Configuration (`/var/ossec/etc/rules/local_rules.xml`):**
  ...
  <group name="ids,suricata,">
    <rule id="100003" level="10">
      <if_sid>86601</if_sid>
      <field name="event_type">^alert$</field>
      <field name="alert.signature">^CUSTOM SQLi Attempt Detected$</field>
      <description>Wazuh Alert: Suricata Custom SQLi Attempt Detected</description>
    </rule>
  </group>
  ```
...
### Wazuh Active Response Configuration (`/var/ossec/etc/ossec.conf` on VM2)

Wazuh Manager is configured with the following active response policies to automatically mitigate threats in real-time:
